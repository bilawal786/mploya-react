    ...
    const displayJobseekers = jobs
        ...
        .map((row) => {
            return (
                ...
                //     </div>
                // </div>
                <div className="col-sm-4 rounded my-3" onClick={() => { singleJob(row.id) }}>
                    <div className="card border-0 shadow-lg nearby-card-dark">
                        <div className="card-body">
                            <div className="text-center">
                                <img className="icon-img text-center mb-2" src={row.employer_image ? "https://mploya.com/" + row.employer_image : "https://i.imgur.com/IRsUTtE.jpg"} alt="pic" /><br />
                                <strong className="text-white">{row.job_title}</strong><br />
                                <small className="primary-span">{row.company_name}</small>
                            </div>
                            <strong>
                                <small className="small-txt text-muted"><i class="fa fa-map-marker fa-sm" aria-hidden="true"></i>&nbsp;{row.address}</small>
                                <small className="small-txt text-muted shrink-0">{row.currencySymbol}&nbsp;{row.min_salary} - {row.max_salary}</small>
                            </strong>
                        </div>
                    </div>
                </div>
